...
#include <iostream>
#include <iomanip> /*formato de nÃºmeros*/
#include <fstream>
...
/*Elimina el Ãºltimo elemento de la lista, es llamado cola*/
void eliminaCola(struct Lista tad) {
    ...
    if (ultimo != nullptr) {
        //Si no hay penultimo, se estÃ¡Â¡ eliminando el Ãšnico nodo de la lista
...
void imprime(struct Lista tad) {
    ...
    cout << setprecision(2);
    if (esListaVacia(tad)) {
        cout << "La lista estÃ¡ vacÃ­a no se puede mostrar" << endl;
        return;
    } else {
        struct Nodo * recorrido = tad.cabeza;
